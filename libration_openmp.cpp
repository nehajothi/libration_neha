...
int main()
{
    ...
    //Set the number of OpenMP threads to be used here. Currently it is 4
    #pragma omp parallel for num_threads(4)
    for(int k = 1 ; k <= ntp ; k++)
    {
        start = omp_get_wtime();
	cout<< "Thread "<< omp_get_thread_num() << " is executing particle " << k << "\n";
	...
	cout<<"time taken by particle " << k << " is " << omp_get_wtime() - start << "\n";
